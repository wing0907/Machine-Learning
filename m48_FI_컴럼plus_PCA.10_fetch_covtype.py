...
for i, fi in enumerate(model.feature_importances_): # index 와 값(fi)
    ...
    if fi <= percentile:        # 값이 낮은 놈을 찾아
        ...
        continue
print(col_name)         # [2, 8, 14, 18, 19, 20, 27, 28, 31, 32, 39, 41, 47, 49]   얘네를 PCA로 압축해서 붙여버리겠드아!

# exit()

x_f = pd.DataFrame(x) # 얘는 위에서 만들었어도 된다
x1 = x_f.drop(columns=col_name)                        # 고놈을 삭제해.
x2 = x_f[[2, 8, 14, 18, 19, 20, 27, 28, 31, 32, 39, 41, 47, 49]]
print(x2)
...
print('FI_Drop + PCA :', model.score(x_test, y_test))         # FI_Drop : 0.48491002814040945
