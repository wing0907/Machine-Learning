...
def load_dataset(name):
    name = name.lower()
    if name in ["california", "02. california", "02", "cal"]:
        data = fetch_california_housing()
        X, y = data.data, data.target
        return X, y
    elif name in ["diabetes", "03. diabetes", "03"]:
        data = load_diabetes()
        X, y = data.data, data.target
        return X, y
    elif name in ["dacon_ddarung", "04. dacon_ddarung", "04"]:
        return load_dacon_ddarung()
    elif name in ["kaggle_bike", "05. kaggle_bike", "05"]:
        return load_kaggle_bike()
    else:
...
def load_dacon_ddarung():
    ...
    _ = pd.read_csv(path + 'submission.csv', index_col=0)  # 사용 안함
...
def load_kaggle_bike():
    ...
    train_csv = pd.read_csv(path + 'train.csv', index_col=0)
    _new_test_csv = pd.read_csv(path + 'new_test.csv', index_col=0)  # 사용 안함
    _submission = pd.read_csv(path + 'sampleSubmission.csv')        # 사용 안함
...
print("=== 전체 조합 결과 ===")
print(
    df.sort_values(by=["dataset", "R2"], ascending=[True, False]).to_string(index=False)
)

# ---------------------------
# 4) 데이터셋별 최고 조합만 추출
#    기준: R² 내림차순 → RMSE 오름차순 → MAE 오름차순
# ---------------------------
sorted_df = df.sort_values(
    by=["dataset", "R2", "RMSE", "MAE"],
    ascending=[True, False, True, True]
)
best_df = sorted_df.groupby("dataset", as_index=False).head(1).reset_index(drop=True)

print("\n=== 데이터셋별 최고 조합만 ===")
print(best_df.to_string(index=False))

# (선택) CSV로 저장하고 싶으면 아래 주석 해제
# df.to_csv("all_results.csv", index=False)
# best_df.to_csv("best_per_dataset.csv", index=False)
