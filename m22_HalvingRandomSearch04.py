...
parameters = [
    {"C": [1, 10, 100], "kernel": ['linear'], "degree": [3, 4]}, 
...
model = HalvingRandomSearchCV(xgb, parameters, cv=kfold,       # 42 * 5 = 210번
                     verbose=1,
                     refit=True, # 다시 훈련                         # 1번
                     n_jobs=1, # -1 = CPU를 FULL로 돌리겠다라는 뜻
                     random_state=333,
...
pd.DataFrame(model.cv_results_).sort_values(
                        'rank_test_score', # rank_test_score를 기준으로 오름차순 정렬
                        ascending=True).to_csv(path + 'm22_04_dacon_ddarung_hgs_cv_results.csv')

path = './_save/m15_cv_results/'
joblib.dump(model.best_estimator_, path + 'm22_04_dacon_ddarung_best_model.joblib')

# best_score (train CV r² 평균) : 0.3419505134167113
# model.score (test r²) : 0.7885586726997871
# MSE : 1423.9842
# R² : 0.7886
# time : 22.3796 seconds
