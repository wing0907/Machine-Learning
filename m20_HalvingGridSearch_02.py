...
model = HalvingGridSearchCV(xgb, parameters, cv=kfold,       # 42 * 5 = 210번
                    ...
                    min_resources=1834,   # 1 iter때의 최소 훈련 행의 개수
                    max_resources=16512, # 데이터 행의 개수(n_samples)
                    )                                              # 총 271번
...
# best_score (train CV r² 평균) : 0.8314821538778308
# model.score (test r²) : 0.8285390039629126
# MSE : 0.2185
# R² : 0.8285
# time : 29.5583 seconds
...
pd.DataFrame(model.cv_results_).sort_values(
                        'rank_test_score', # rank_test_score를 기준으로 오름차순 정렬
                        ascending=True).to_csv(path + 'm20_02_california_hgs_cv_results.csv')

path = './_save/m15_cv_results/'
joblib.dump(model.best_estimator_, path + 'm20_02_california_best_model.joblib')
