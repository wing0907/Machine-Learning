...
for i, fi in enumerate(model.feature_importances_): # index 와 값(fi)
    ...
    if fi <= percentile:        # 값이 낮은 놈을 찾아
        ...
        continue
print(col_name)         # ['AveBedrms', 'Population']   얘네를 PCA로 압축해서 붙여버리겠드아!
...
model.fit(x_train, y_train)                                   # r2 :  0.83707103301617
print('FI_Drop + PCA :', model.score(x_test, y_test))         # FI_Drop : 0.8387939858950988
                                                              # FI_Drop + PCA : 0.8374852395815212
