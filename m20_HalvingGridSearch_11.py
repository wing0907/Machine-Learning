...
model = HalvingGridSearchCV(xgb, parameters, cv=kfold,       # 42 * 5 = 210번
                    ...
                    # min_resources=100,   # 1 iter때의 최소 훈련 행의 개수
                    # max_resources=1437, # 데이터 행의 개수(n_samples)
                    )                                              # 총 271번
...
pd.DataFrame(model.cv_results_).sort_values(
                        'rank_test_score', # rank_test_score를 기준으로 오름차순 정렬
                        ascending=True).to_csv(path + 'm20_11_digits_hgs_cv_results.csv')

path = './_save/m15_cv_results/'
joblib.dump(model.best_estimator_, path + 'm20_11_digits_best_model.joblib')

# best_score :  0.944239602731223
# model.score :  0.9472222222222222
# best_acc_score :  0.9472222222222222
# accuracy_score :  0.9472222222222222
# time :  1116.2193 seconds