...
# 2. 모델구성
model = XGBRFRegressor(random_state=seed)
...
for i, fi in enumerate(model.feature_importances_): # index 와 값(fi)
    ...
    if fi <= percentile:        # 값이 낮은 놈을 찾아
        ...
        continue
print(col_name)         # ['holiday', 'weather', 'windspeed']

x = pd.DataFrame(x, columns=x_train.columns) # 얘는 위에서 만들었어도 된다
...
model.fit(x_train, y_train)
print('acc :', model.score(x_test, y_test))         # acc : 0.9973445534706116
