...
model = HalvingRandomSearchCV(xgb, parameters, cv=kfold,       # 42 * 5 = 210번
                    ...
                    min_resources=50,   # 1 iter때의 최소 훈련 행의 개수
                    max_resources=455, # 데이터 행의 개수(n_samples)
                    )                                              # 총 271번
...
pd.DataFrame(model.cv_results_).sort_values(
                        'rank_test_score', # rank_test_score를 기준으로 오름차순 정렬
                        ascending=True).to_csv(path + 'm22_07_dacon_diabetes_hgs_cv_results.csv')

path = './_save/m15_cv_results/'
joblib.dump(model.best_estimator_, path + 'm22_07_dacon_diabetes_best_model.joblib')
