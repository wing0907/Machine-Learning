...
# 최적의 매개변수 :  {'max_depth': 6, 'learning_rate': 0.1}
...
pd.DataFrame(model.cv_results_).sort_values(
                        'rank_test_score', # rank_test_score를 기준으로 오름차순 정렬
                        ascending=True).to_csv(path + 'm18_06_cancer_rs_cv_results.csv')

path = './_save/m15_cv_results/'
joblib.dump(model.best_estimator_, path + 'm18_06_cancer_best_model.joblib')
